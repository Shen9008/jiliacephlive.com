...
.DESCRIPTION
  ...
  Full https://jiliacephlive.com/... URLs in sitemap are normalized to root-relative paths for disk checks.
...
function Resolve-SiteUrlToPath {
    ...
    # Strip fragment/query, then normalize known production hosts to path-only (sitemap + internal checks).
    $p = $UrlPath -replace '#.*$', '' -replace '\?.*$', ''
    $p = $p -replace '^https?://pgasiagames\.com', ''
    $p = $p -replace '^https?://jiliacephlive\.com', ''
    if ($p -eq "" -or $p -eq "/") {
