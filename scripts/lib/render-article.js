...
const { validatePost, DEFAULT_BLOG_IMAGE } = require('./normalize-post.js');
const { injectInternalLinks } = require('./inject-internal-links.js');
const { blogPostUrl, siteBaseUrl } = require('./site-url.js');
...
/** Absolute URL for og:image / JSON-LD (Strapi may return relative or absolute). */
function absoluteFeaturedImageUrl(origin, url) {
  const u = String(url || '').trim();
  if (!u) return `${origin}${DEFAULT_BLOG_IMAGE}`;
  if (/^https?:\/\//i.test(u)) return u;
  const pathname = u.startsWith('/') ? u : `/${u}`;
...
function renderArticle(normalized, opts = {}) {
  ...
  const baseUrl = blogPostUrl(normalized.slug);
  const origin = siteBaseUrl();
  const featuredSrc = String(normalized.featured_image || '').trim() || DEFAULT_BLOG_IMAGE;
  const ogImageUrl = absoluteFeaturedImageUrl(origin, featuredSrc);
  ...
  const replacements = {
    ...
    '{{FEATURED_IMAGE}}': escapeHtml(featuredSrc),
    '{{IMAGE_ALT}}': escapeHtml(normalized.title || ''),
