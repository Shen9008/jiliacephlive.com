...
function unwrapStrapiMedia(val) {
  ...
  if (typeof val === 'object') {
    ...
    const d = val.data;
    if (d == null) return '';
    const inner = Array.isArray(d) ? d[0] : d;
...
function pickFeaturedImageUrl(post) {
  const keys = [
    'cover',
    'featured_image',
    'featuredImage',
  ...
  for (const k of keys) {
    const u = unwrapStrapiMedia(post[k]);
    if (u) return u;
  }
  return '';
}
...
function normalizePost(strapiPost, opts = {}) {
  ...
  let featured_image = pickFeaturedImageUrl(strapiPost);
  if (!featured_image) featured_image = DEFAULT_BLOG_IMAGE;
