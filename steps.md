...
2. **`npm install dotenv sharp`** — add **`wrangler`** if you deploy with the CLI (this repo’s `package.json` matches that set; add **`glob`** / **`form-data`** only if you extend scripts that need them).
...
```bash
npm install dotenv sharp wrangler --save-dev
```

(Optional extras if your fork adds tooling: `glob`, `form-data`.)
...
### Visitor-facing copy (no pipeline jargon)

Anything **shown to normal site visitors** on the blog index, article shells, or listing scripts must read like editorial product copy. **Do not** tell visitors to run **`npm`**, name **CLI commands**, mention **Strapi / CMS**, **sync jobs**, **`blogs.json`**, or other implementation details in UI strings, meta descriptions, or empty states.

Recommended patterns:

| Situation | OK | Avoid |
|-----------|-----|--------|
| No posts yet | Friendly “coming soon” + links to main site sections | “Run `npm run sync`”, “configure Strapi”, “empty `blogs.json`” |
| Feed failed to load | Short apology + refresh or link home | Paths like `/assets/data/...`, stack traces, CMS names |

Operational instructions belong **only** in this guide, `.env.example`, and **logs** from local runs or CI—not in the HTML/JS users see.
...
- `#blog-pagination-truncated` — optional `<p hidden>`; shown when the feed has **more than 594** posts; explains that only the most recent **594** appear on this listing. Keep copy **editorial** (same rule as **Visitor-facing copy**—no npm/CMS/sync jargon).
...
- Requires styles for `.blog-pagination` and related classes (this repo: `css/components.css` via `css/main.css`).
...
Deploy secrets (`CLOUDFLARE_*`) are only needed if an Actions workflow deploys to Pages; this repo’s **`daily-sync`** workflow runs **`sync`** + **`backfill:force`** only—you **commit and push** generated files yourself (or use the separate manual deploy workflow).
